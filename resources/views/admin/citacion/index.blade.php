<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <x-navbars.navs.auth titlePage="Citaciones"></x-navbars.navs.auth>
            ...
            <script>
                $(document).ready(function() {
                    Swal.fire({
                        position: "center",
                        icon: "success",
                        title:  "{{ session('correcto') }}", //"Operación Realizada con éxito",
                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body p-3">
                            ...
                            <a type="button" class="btn btn-dark" href="{{ route('citacion.create') }}">
                            ...
                            <div class="tab-content">
                                <div class="table-responsive-xl">
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col" class="px-4 py-3">Nro. Citación</th>
                                        <th scope="col" class="px-6 py-3">Descripción</th>
                                        <th scope="col" class="px-6 py-3">Registro de Atención</th>
                                        <th>Estado</th>
                                        <th class="text-end">Acciones</th>
                                    </tr>
                                </thead>
                                <tbody>
                                            @foreach ($citaciones as $item)
                                                <tr>
                                                    <td>{{ $loop->iteration }}</td>
                                                    <td>{{ $item->fecha }}</td>
                                                    <td>{{ $item->numero_citacion }}</td>
                                                    <td class="text-truncate" style="max-width: 200px">{{ $item->descripcion }}</td>
                                                    <td class="text-truncate text-center" style="max-width: 200px">
                                                        <span title="número de caso">{{ $item->registro_de_atencion->nro_caso }}</span> <br>
                                                        <span title="petición">{{ $item->registro_de_atencion->peticion }}</span>
                                                    </td>
                                                    <td>
                                                        @if ($item->estado == 1)
                                                    ...
                                                    <td >
                                                        @if ($item->estado == 1)
                                                            <a href="{{ route('citacion.show', $item->id) }}"
                                                                class="btn-sm btn-info" title="Ver detalles"> <i
                                                                    class="far fa-eye"></i></a>
                                                            <a href="{{ route('citacion.edit', $item->id) }}"
                                                            ...
                                                            <form
                                                                action="{{ route('citacion.delete', $item->id) }}"
                                                        ...
                                                        @else
                                                            <a href="{{ route('citacion.reingresar', $item->id) }}"
